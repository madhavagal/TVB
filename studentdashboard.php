...
<?php  
    $connect = mysqli_connect("localhost", "root", "", "tvb");
    $id=$_SESSION['id'];
    $query1 = "SELECT name,email,photo,phno from student WHERE id=$id";
    ...
    $result2 = mysqli_query($connect, $query2);
    $i = 0;
    $dyn_table = "<center><table cellpadding=\"20\">"; 
     while($row2 = mysqli_fetch_array($result2))  
     {
       ...
       $cname = $row3['name'];
       $image = $row3['photo'];
       $query4 = "SELECT sdate,active FROM batches WHERE batchid = $bid AND courseid = $cid";
       ...
       $msg;
       if($active == 1){
         $msg = "<p class=\"notcompleted\">Course Active</p>";
       }
       else{
         $msg = "<p class=\"notstarted\">Course Inactive</p>";
       }
       if ($tdate > $sdate and $active != 1)
       {
       ...
       elseif ($tdate < $sdate and $active != 1)
       {
         ...
         $query5 = "SELECT classno,classdate,topic,teacherid FROM courseclass WHERE courseid = $cid AND batchid = $bid AND classdate >= CURRENT_DATE ORDER BY classdate ASC LIMIT 1";
         ...
         $cno = $row5['classno'];
         $tid = $row5['teacherid'];
         $query6 = "SELECT name FROM teacher WHERE id = $tid";
         $result6 = mysqli_query($connect, $query6);
         $row6 = mysqli_fetch_array($result6);
         $tname = $row6['name']; 
         $upclass .= "<li><h5 style='font-size:20px;'>$cname - Class $cno by $tname <br><small>Date - $update</small><br><small>Topic - $topic</small></h5></li>";
         }

       }
                                    //   echo "$msg ";
        $img = '<img src="data:image/jpeg;base64,'.base64_encode($image ).'" height="130px" width="100%" class="img-thumnail" />';
        $div = "<div class=\"card arrow\" style=\"width: 17rem; height:15rem;\">
                 $img
                 <div class=\"card-body\">
                 <h4 class=\"card-title\" style='color:white;'>$cname - Batch $bid</h4>
                 $msg
                 </div>
                 </div>";
    ...
    if(empty($upclass)){
      $upclass = "<center><h5>No upcoming classes</h5></center>";
    }
    $upclass .= '</ul>'; 
     ?>
...
<head>
  ...
  <link href="stddash.css?ver=<?=time();?>" rel="stylesheet" />
          ...
          <div class="row">
            <div class="col-md-8">
              <div class="card">
                <div class="header">
                  <center>
                    <h3 class="title nomargin" style="margin-top:22px;">Your Courses : </h3>
                  </center>
                </div>
                <div class="content">
                  <?php echo $dyn_table; ?>
...
<!--                    </a>-->
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card">
                <div class="header">
                  <h3 class="title" style="margin-left:22px;">Upcoming Classes : </h3>
                  <br>
                </div>
                <div class="content">
                  <?php echo $upclass; ?>
  ...
  <div class="modal fade" id="updatepropicmodal" tabindex="-1" role="dialog" aria-labelledby="updatepromodal" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="post" action="studentdashboard.php" enctype="multipart/form-data">
            <div class="form-group">
