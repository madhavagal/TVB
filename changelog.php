...
<?php
$connect = mysqli_connect("localhost","root","","tvb");
$query1 = "SELECT adminid,comment,timestamp from changelog ORDER BY timestamp DESC";
$result1 = mysqli_query($connect, $query1);
$dyn_table = '<div class="scrollable" style="height:600px;">
...
while($row1 = mysqli_fetch_array($result1)){
  $admid = $row1['adminid'];
  $com = $row1['comment'];
  $tstamp = $row1['timestamp'];
  $dynrow = '<tr><td>'.$admid.'</td><td>'.$com.'</td><td>'.$tstamp.'</td></tr>';
  $dyn_table .= $dynrow;
}
...
<head>
  ...
  <link href="assets/css/light-bootstrap-dashboard.css?ver=<?=time();?>" rel="stylesheet" />
  <link href="adminstd.css" rel="stylesheet" />
...
<body>
  <div class="wrapper">
    ...
    <div class="main-panel">
      ...
      <div class="content">
        <div class="container-fluid">
          <div class="section">
            <button class="btn btn-danger" data-toggle="modal" data-target="#delchangemodal">Empty the Changelog</button>
            <br><br>
            <?php echo $dyn_table; ?>
          </div>
        </div>
  ...
  <div class="modal fade" id="delchangemodal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Empty the Changelog</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <p style="color:black; font-size:18px;">Are you sure you want to delete all entries in the Changelog?</p>
          <p style="color:red; font-size:12px;">Warning: This action is irreversible</p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
          <button type="button" class="btn btn-danger" id="delchbtn">Delete</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    $(document).ready(function() {
      $("#delchbtn").click(function(e) {
        var url = "emptychange.php";
        $.ajax({
          type: "POST",
          url: url,
          success: function(data) {
